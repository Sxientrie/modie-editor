...
class SearchRoutesMixin:
    ...
    def _api_search_files(self):
                                    ...
                                    try:
                                        ...
                                        if sdcard_root:
                                            try:
                                                ...
                                                rel_path = "storage_external/" + str(rel) if str(rel) != "." else "storage_external"
                                            except ValueError:
                                                pass
                                if rel_path:
                                    results.append({
        ...
        # Why: Run recursive directory search on a background thread to prevent blocking request connection thread
        t = threading.Thread(target=run_search_thread, daemon=True)
        t.start()

        while not status["done"]:
            time.sleep(0.01)
            # Why: Check if client connection has disconnected to abort the background search thread immediately
            if hasattr(self, "connection") and self.connection is not None:
                try:
                    r, _, _ = select.select([self.connection], [], [], 0)
                    if r:
                        # Why: Any readability (new data or EOF) on a GET request socket indicates the client has 
                        # either disconnected or reused the socket for a new request. In either case, the current 
                        # search is cancelled.
                        status["cancel"] = True
                        break
                except Exception:
